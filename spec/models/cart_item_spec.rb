...
describe CartItem do
  context "CartItem" do
    before(:each) do
      ...
      @cart_item = CartItem.new(variant_id: @variant.id, item_type_id: 1, quantity: 2)
    end
...
describe Cart, " instance methods" do
  before(:each) do
    @variant = Factory(:five_dollar_variant)
    @cart_item = CartItem.new(variant_id: @variant.id, item_type_id: ItemType::SHOPPING_CART_ID)
  end
  context " price" do
